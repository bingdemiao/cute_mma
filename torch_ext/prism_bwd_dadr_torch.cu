...
std::vector<torch::Tensor> prism_backward_dA_dR(
    ...
    double dropout_p)
{
    TORCH_CHECK(B.is_cuda() && B.scalar_type() == k_prism_torch_dtype && B.is_contiguous(),
                "B must be a contiguous CUDA ", k_prism_dtype_name, " tensor");
    ...
    // Required because prism_backward_dA_dR_launch internally calls cudaMalloc,
    // which uses the current device — concurrent calls from different threads
    // on different devices would otherwise race and trigger illegal memory
    // accesses when kernel launches see pointers from the wrong device.
    ...
    torch::Tensor d_ib;
    prism_native* d_ib_ptr = nullptr;
    if (CurrKernelParams::internal_bias) {
        d_ib = torch::zeros({n_groups, k}, A.options());
        d_ib_ptr = PRISM_DATA(d_ib);
    }

    cudaStream_t stream = at::cuda::getCurrentCUDAStream(A.device().index()).stream();

    // Optional shuffle: build A_perm (n_groups, M, K) and route the kernel's
    // dA into a per-group dA_perm buffer; post-kernel inverse-shuffle + sum.
    const prism_native* A_kernel_ptr = PRISM_CONST_DATA(A);
    prism_native* dA_kernel_ptr = PRISM_DATA(dA);
    int strideA_per_group = 0;
    int strideDA_per_group = 0;
    torch::Tensor A_perm_buf;
    torch::Tensor dA_perm_buf;
    ...
    if (shuffle_masks.has_value()) {
        ...
        n_chunks = masks.size(1);
        n_rounds = masks.size(2);
        ...
        dA_perm_buf = torch::zeros({n_groups, m, k}, opts);
        ...
        dA_kernel_ptr = PRISM_DATA(dA_perm_buf);
        strideA_per_group = (int)(m * k);
        strideDA_per_group = (int)(m * k);
    }
    ...
    // Post-kernel: inverse-shuffle each per-group dA_perm[g] and accumulate
    // into dA. dA was zero-initialized.
    if (shuffle_masks.has_value()) {
        const auto& masks = shuffle_masks.value();
        const int64_t* masks_ptr = masks.data_ptr<int64_t>();
        prism_native* dA_perm_ptr = PRISM_DATA(dA_perm_buf);
        prism_native* dA_ptr_h = PRISM_DATA(dA);
        int threads = 256;
        int64_t total_warps = m * n_chunks;
        int blocks = (int)((total_warps * 32 + threads - 1) / threads);
        for (int64_t g = 0; g < n_groups; ++g) {
            prism_shuffle::shuffle_inverse_add_kernel<prism_native><<<blocks, threads, 0, stream>>>(
                dA_ptr_h,
                dA_perm_ptr + g * m * k,
                masks_ptr + g * n_chunks * n_rounds,
                m, k, n_chunks, n_rounds);
